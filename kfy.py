    ...
    def stats_reporter(self) -> None:
        ...
        while self.running:
            try:
                 ...
                 logger.info
                 print("""
                    \033[38;5;220mStats: Time={elapsed_total:.1f}s |Req={current_req_count}|)
                    \033[38;5;206mSuccess={current_success}({success_rate:.1f}%)|)
                    \033[32mErrors={current_errors}({error_rate:.1f}%)[ConnErrs={current_conn_err}]|)
                    \033[37mRPS={rps_interval:.2f} (avg: {rps_total:.2f})|)
                    \033[38;5;37mSent={mb_sent:.2f} MB ({mbps:.2f} Mbps))
                    """)
