    ...
    def stats_reporter(self) -> None:
        ...
        while self.running:
            try:
                 ...
                 logger.info(
                    f"\033[38;5;220mStats: Time={elapsed_total:.1f}s \033[37mReq={current_req_count}")
                 logger.info(
                    f"\033[38;5;206mSuccess={current_success} \033[37m({success_rate:.1f}%)")
                 logger.info(
                    f"\033[32mErrors={current_errors}({error_rate:.1f}%) \033[37m[ConnErrs={current_conn_err}]")
                 logger.info(
                    f"\033[37mRPS={rps_interval:.2f} \033[37m(avg: {rps_total:.2f})")
                 logger.info(
                    f"\033[38;5;37mSent={mb_sent:.2f} MB \033[37m({mbps:.2f} Mbps)")
